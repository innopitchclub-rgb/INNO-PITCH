...
const ADMIN_PASSWORD = 'DK14112007';
...
function normalizeRegistrationRecord(row = {}) {
  ...
  normalized.Email = normalizeRecipientEmail(row.Email || row.email || row.studentEmail || row['Email Address'] || row['email address'] || '');
...
async function sendGreetingEmail(studentName, studentEmail) {
  const resolvedName = sanitizeText(studentName);
  const resolvedEmail = normalizeRecipientEmail(studentEmail);

  console.log('Student Name:', resolvedName);
  console.log('Student Email:', resolvedEmail);

  if (!resolvedEmail || resolvedEmail.trim() === '') {
    showToast('Student email address is missing.', 'error');
    return false;
  }
  ...
  if (typeof emailjs === 'undefined' || typeof emailjs.send !== 'function') {
    showToast('❌ Greeting email failed.', 'error');
    return false;
  }

  try {
    await emailjs.send('service_3s6k61q', 'template_z7dg3ls', templateParams);
    showToast('✔ Greeting email sent successfully.', 'success');
    return true;
  } catch (error) {
    const detail = error?.text || error?.message || error?.response?.data?.message || 'Unknown EmailJS error';
    console.error('Greeting email failed:', detail);
    showToast('❌ Greeting email failed.', 'error');
    return false;
  }
}
...
function renderSummary(data) {
  ...
  setCount('countPending', pending);
  setCount('countApproved', approved);
  setCount('countRejected', rejected);
}

function filterRegistrations(rows, searchTerm) {
...
async function initAdminDashboard() {
  ...
  function applySearch() {
    ...
    const filtered = filterRegistrations(allRows, term);
  ...
  document.getElementById('registrationsBody')?.addEventListener('click', async (event) => {
    ...
    try {
      if (action === 'approve') {
        const resolvedEmail = normalizeRecipientEmail(row.Email || row.email || row['Email ID'] || row['Email Address'] || '');
        console.log('Approval email recipient:', resolvedEmail);
        await postAdminAction('update', id, { Status: 'Approved' });

        if (resolvedEmail && resolvedEmail.trim() !== '') {
          await sendGreetingEmail(row.Name, resolvedEmail);
        } else {
          showToast('Student email address is missing.', 'error');
        }
